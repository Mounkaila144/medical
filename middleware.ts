...
const publicTenantRoutePatterns = [
  /^\/[^/]+\/queue\/take-number$/,  // /[tenantSlug]/queue/take-number
  /^\/[^/]+\/queue\/display$/,      // /[tenantSlug]/queue/display
];

export function middleware(request: NextRequest) {
  const { pathname } = request.nextUrl;

  // Check if the route is public (exact or starts with)
  if (publicRoutes.some(route => pathname.startsWith(route))) {
    return NextResponse.next();
  }

  // Check if the route matches public tenant patterns
  if (publicTenantRoutePatterns.some(pattern => pattern.test(pathname))) {
    return NextResponse.next();
  }
