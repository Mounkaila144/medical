...
class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      // Handle empty responses (204 No Content, or 201 with no body)
      const contentType = response.headers.get('content-type');
      const contentLength = response.headers.get('content-length');

      // If status is 204 or content-length is 0, return null/undefined
      if (response.status === 204 || contentLength === '0') {
        return null as T;
      }

      // If there's no content-type or it's not JSON, try to parse as JSON anyway
